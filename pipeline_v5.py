...
def process_file(src_str, dst_root_str, base_str):
    src = Path(src_str)
    ...
    best = min(valid, key=lambda x: x["size"]) if valid else candidates[0]
...
def main():
    if len(sys.argv) < 3:
        print("usage: python pipeline_v5.py input_dir output_dir")
        return
    ...
    print(f"FILES: {len(files)} / WORKERS: {max_workers}")

    results = []

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_file, f, str(output_dir), str(input_dir))
            for f in files
        ]
